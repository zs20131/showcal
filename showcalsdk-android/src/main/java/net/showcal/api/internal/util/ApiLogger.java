...
//        blog.error(sb.toString());
     ...
     * 发生特别错误时记录完整错误现场
     */
    public static void logErrorScene(RequestParameters context, XiniuResponse xRsp, String appSecret) {
        if(!needEnableLogger) {
            return;
        }
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        ...
        sb.append("^_^");
        sb.append(xRsp.getErrorCode());
        sb.append("^_^");
//        sb.append(xRsp.getSubCode());
//        sb.append("^_^");
...
//        blog.error(sb.toString());
    ...
    private static void appendLog(XiniuHashMap map, StringBuilder sb) {
        boolean first = true;
