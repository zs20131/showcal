...
public class Detail {
    ...
    public void execute(Context context, @Param("id") Long id) throws Exception {
        ...
        Integer countService = userGetResponse.getCountService();
        if (user != null) {
        ...
        context.put("countService", countService);
        //所有发布
        ...
        commentPostFindRequest.setThreadId(id);
        ...
        for (CommentPost commentPost : commentPostFindResponse.getResult()) {
            if(commentPost!=null&&commentPost.getGrade()!=null&&commentPost.getGrade()!=0){
        ...
        context.put("userId",id);
            context.put("exist", true);
    }
        else{
            context.put("message", "瘦咖不存在");
            context.put("exist", false);
        }
    }
